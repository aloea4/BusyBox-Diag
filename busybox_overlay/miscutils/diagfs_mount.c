                    ...
                    int quiet)
{
    ...
    if (!fp) {
        fprintf(stderr, "Error: failed to read /proc/self/mounts\n");
    ...
    if (out_format == DIAGFS_OUT_JSON) {
        ...
        printf("diagfs - Scanning all mounted filesystems\n");
        printf("------------------------------------------\n");
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (diag_fs_read(entry.target, &fs) != DIAG_OK) {
            if (!quiet)
                fprintf(stderr, "Warning: failed to read mount point %s; skipped.\n", entry.target);
            continue;
        }
